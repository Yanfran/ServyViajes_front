...
export class EditComponent {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.comprobantes = this.sharedData.reserve_payments;
    ...
    this.fechaEntrada = this.datePipe.transform(this.sharedData.fecha_entrada, 'dd/MM/yyyy');
    this.fechaSalida = this.datePipe.transform(this.sharedData.fecha_salida, 'dd/MM/yyyy');



    // const planId = this.sharedData.plan_id;
    // this.createForm.get('planes').setValue(planId);
    ...
    if (this.sharedData) {
      ...
      this.programaForm.patchValue({
        clave_reservacion: this.sharedData.clave_reservation,
        observaciones: this.sharedData.observaciones,
      });
    }
    ...
    this.createForm.get('estatus').valueChanges.subscribe(async value => {
      if (value === 1) {
        // console.log("hola", value)
        const codigo = this.generarCodigo();
        // Haz algo con el código aquí
        this.programaForm.get('clave_reservacion').setValue(codigo);
      }
    });
  ...
  async planId(id: number): Promise<void> {    
    try {
      ...
      const hotelSelected = this.data.find(it => it.hotel_id === this.idHotel);

      if(hotelSelected && hotelSelected.hotel.rooms_by_hotels.length > 0){
        for (const reservation_room of this.reservation_rooms) {
          const matchingRoom = hotelSelected.hotel.rooms_by_hotels.find(it => it.id === reservation_room.room_id);
          if (matchingRoom) {
            const exist = this.selectedPlanData.find(it => it.id === matchingRoom.id);
            if(!exist){
              this.selectedPlanData.push(matchingRoom);
            }
          }
        }
      }
  ...
  async edit(): Promise<void> {
    ...
    var datos = {
      ...
      clave_reservation: this.programaForm.value.clave_reservacion,
    };
